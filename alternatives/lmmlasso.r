get_lmmlasso_function <- function(x, y, z, grp, lambda){
  library(lmmlasso)
  fit2 <- lmmlasso(x=x,y=y,z=z,grp=as.factor(grp),lambda=lambda,pdMat="pdDiag")
  mylist <- list("beta" = fit2$fixef,
                 "gamma" = fit2$Psi,
                 "aic" = fit2$aic,
                 "bic" = fit2$bic,
                 "y_pred" = fit2$fitted.values)
  return(mylist)
}
...
# data(classroomStudy)
# fit1 <- lmmlasso(x=classroomStudy$X,y=classroomStudy$y,z=classroomStudy$Z, grp=classroomStudy$grp,lambda=15,pdMat="pdIdent")
# summary(fit1)
# plot(fit1)
...
#
# mylist <- get_lmmlasso_function(x, y, z, grp, 10)
